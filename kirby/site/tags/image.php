<?php
// this is a copy of the original kirby image tag
// I've only changed line 81 - adding .img-responsive class for all images per default
  ...
  'html' => function($tag) {
    ...
    $_image = function($class) use($tag, $url, $alt, $title) {
      return html::img($url, array(
        ...
        'class'  => 'img-responsive'.( (strlen($class) > 0) ? ' '.$class : ''),
        'title'  => $title,
        'alt'    => $alt
      ));
    };
    ...
    if(kirby()->option('kirbytext.image.figure') or !empty($caption)) {
      ...
      $figure->addClass('lso-figure');
      $figure->append($image);
      if(!empty($caption)) {
        $figure->append('<figcaption>' . html($caption) . '</figcaption>');
      } else if (!empty($file->captionHeader())) {
        $figure->append('<figcaption class="lso-figure-caption"><div class="lso-figure-caption-header">' . html($file->captionHeader()) . '</div><div class="lso-figure-caption-body">' . html($file->captionBody()) . '</div></figcaption>');
      }
      return $figure;
    } else {
